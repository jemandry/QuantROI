...
class ReminderBot:
    ...
    def check_milestones(self) -> List[str]:
        ...
        for milestone in self.milestones:
            ...
            if (time_until_deadline <= timedelta(hours=48) and
                    time_until_deadline > timedelta(0)):
                alerts.append(
                    f"⏰ Milestone '{milestone.name}' due in "
                    f"{time_until_deadline.days} days"
                )

            elif time_until_deadline <= timedelta(0):
                alerts.append(
                    f"🚨 Milestone '{milestone.name}' is overdue by "
                    f"{abs(time_until_deadline.days)} days"
                )
    ...
    def process_milestone_payment(self, milestone_id: str, approved: bool) -> bool:
        ...
        if approved:
            ...
            self.log_to_solana("milestone_payment_approved", payment_data)

            milestone.approved = True
            milestone.completed = True

            print(f"✅ Payment approved for milestone '{milestone.name}': "
                  f"{milestone.payment_amount} {milestone.currency}")
            return True
        else:
            rejection_data = {
                "milestone_id": milestone_id,
    ...
    def run_daily_check(self) -> None:
        ...
        if failures:
            ...
            for failure in failures:
                ...
                failure_summary[component].append(
                    f"{failure.tool}: {failure.errors[0]}"
                )
